package com.example.tuantran.ttplayer.ui.splash;
...
public class SplashActivity extends AppCompatActivity implements EasyPermissions.PermissionCallbacks,
    ...
    public void readAndWriteStore() {
        if (hasLocationAndContactsPermissions()) {
            ...
            loadLocalSongs();
            AppDataManager.getInstance().setDataLocalSong(data);
            AppDataManager.getInstance().setDataPlaylist(data);
    ...
    private void loadLocalSongs() {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                ...
                data.add(new SongModel(id, artist, title, displayName, streamUri, albumId, date, duration, fileSize));
            }
            cursor.close();
        }
    }
